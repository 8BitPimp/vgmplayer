...
static bool _vgm_parse_single(struct vgm_context_t* vgm, uint32_t* delay)
{
    ...
    const uint8_t opcode = vgm->stream->read8();
    switch (opcode) {
    case (0x4f):
    case (0x50): {
        // write to sn76489
        const uint8_t data1 = stream->read8();
    ...
    case (0x52): {
        ...
        const uint8_t data1 = stream->read8();
        const uint8_t data2 = stream->read8();
    ...
    case (0x61): {
        // wait dd dd samples
        const uint16_t samples = stream->read16();
    ...
    case (0x67): {
        // data block
        uint8_t data1 = stream->read8();
        assert(data1 == 0x66);
        const uint8_t tt = stream->read8();
        const uint32_t ss = stream->read32();
    ...
    struct vgm_chip_bank_t* chips)
{
    ...
    vgm->stream->read(&(vgm->header), vgm_hdr_size);
    uint32_t to_skip = 0;
    // check VGM header
...
uint32_t vgm_delay_ms(struct vgm_context_t* vgm)
{
    assert(vgm);
    return _vgm_to_millis(vgm, vgm->state.delay);
}

uint32_t vgm_delay_samples(struct vgm_context_t* vgm)
{
    assert(vgm);
    return vgm, vgm->state.delay;
}
